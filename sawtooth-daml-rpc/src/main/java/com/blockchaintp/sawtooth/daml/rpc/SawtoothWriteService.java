...
public final class SawtoothWriteService implements WriteService {
  ...
  private Future sendToValidator(final Batch batch) {
    ...
    synchronized (this) {
      streamToValidator = this.stream.send(Message.MessageType.CLIENT_BATCH_SUBMIT_REQUEST,
          cbsReq.toByteString());
    }
  ...
  private Map.Entry<String, ClientBatchStatus.Status> checkBatchWaitForTerminal(
    ...
    while (true) {
      try {
        ...
        Future cbstatFut;
        synchronized (this) {
          cbstatFut = this.stream.send(MessageType.CLIENT_BATCH_STATUS_REQUEST, cbstatReq.toByteString());
        }
